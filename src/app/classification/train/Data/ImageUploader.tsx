...
const ImageUploader = ({setData}:ImageUploaderProps) => {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    const sample = imageFiles.slice(0, 8).map((file) => (URL.createObjectURL(file)))
    setImageInfo({
  ...
  return (
    <Flex {...getRootProps()} $width='35rem' $height='15rem' style={isDragActive ? {border:'2px dashed#4F95D2'}:{border:'2px dashed#ffffff'}}>
      <Flex $flex='2.2' $flex_direction='column' $align_items='center'>
        ...
        <Text $fontSize='1.5rem' $marginTop='1rem' $variants='Small'>Drop your image folder</Text>
        ...
        {imageInfo && (
          <>
            ...
            <Flex $marginTop='1.5rem' $width='90%' $justify_content='center' $flex_wrap='wrap'>
              {imageInfo?.sample.map((img, key) => {
                return <Image width={25} height={25} style={{ borderRadius: '10px', margin: '0.2rem' }} key={key} alt='' src={img}></Image>
              })}
            </Flex>
          </>
        )}
